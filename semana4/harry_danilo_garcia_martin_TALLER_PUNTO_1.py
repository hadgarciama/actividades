'''
AyudanteNomina v2.0a
Programa para calcular la nomina de una empresa. Lee los nombres desde un archivo de texto y los salarios desde otro archivo de texto. Los nombres de los 2 archivos se deben suministrar como argumentos en la linea de comando al ejecutar el programa. El primer argumento para los nombres y el segundo para los salarios.
...
lineas_archivo_nombres = leer_lineas_archivo(nombre_archivo_nombres)
lineas_archivo_salarios = leer_lineas_archivo(nombre_archivo_salarios)
...
guardar_log("Archivos de nombres y salarios leidos correctamente")
linea1_archivo_nombres=lineas_archivo_nombres[0].split("\xbf")
lineas_archivo_nombres[0]=linea1_archivo_nombres[1]
for y in range(0,numero_lineas_nombres):
	if lineas_archivo_nombres[y].endswith("\n"):
		lineas_archivo_nombres_sin_enter=lineas_archivo_nombres[y].split(".")
		lineas_archivo_nombres[y]=lineas_archivo_nombres_sin_enter[0]
linea1_archivo_salarios=lineas_archivo_salarios[0].split("\xbf")
lineas_archivo_salarios[0]=linea1_archivo_salarios[1]
for y in range(0,numero_lineas_salarios):
	if lineas_archivo_salarios[y].endswith("\n"):
		lineas_archivo_salarios_sin_enter=lineas_archivo_salarios[y].split(".")
		lineas_archivo_salarios[y]=lineas_archivo_salarios_sin_enter[0]
#	lineas_archivo_salarios[y]=int(lineas_archivo_salarios[y])
#validar que los 2 archivos tengan la misma cantidad de lineas
if (numero_lineas_nombres!=numero_lineas_salarios):
	terminar_programa("el archivo "+nombre_archivo_nombres+" y "+nombre_archivo_salarios+" deben tener a misma cantidad de lineas")
# Validar que el archivo nombres tenga el minimo numero de lineas.
...
#con los 2 archivos, crear un solo archivo llamado nomina.txt para continuar con la misma logica.
array_nomina = [[col for col in range (1)] for row in range(numero_lineas_nombres)]
for x in range(0,numero_lineas_nombres):
	array_nomina[x]=lineas_archivo_nombres[x]+"*"+lineas_archivo_salarios[x]
for x in range(0,numero_lineas_nombres):
	linea=array_nomina[x]+"\n"
	escribir_linea_archivo("nomina.txt",linea)
nombre_archivo_nomina="nomina.txt"
