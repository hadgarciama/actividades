'''
AyudanteNomina v2.0e
Programa para calcular la nomina de una empresa. Lee los nombres, salarios y cargos desde un archivo de texto cuyo nombre se ingresa como argumento en la linea de comandos.Luego suma la nomina total de la empresa y suma la nomina por cargos.Tambien calcula promedio de suledos y promedio de sueldos por area.
Todos los resultados aparecen en el archivo liquidacion.txt.
Desarrollado por Harry Danilo Garcia Martin hadgarciama@unal.edu.co
...
#La cuarta posicion contiene el cargo del empleado al cual no hay que validarlo
def validar_linea(linea_por_validar, numero_linea):	
	...
	if (arreglo_campos[2].startswith("ingeniero") or arreglo_campos[2].startswith("tecnico") or arreglo_campos[2].startswith("tecnologo") or arreglo_campos[2].startswith("lider") or arreglo_campos[2].startswith("gerente")):
		array_respuesta[3] = arreglo_campos[2]
	return array_respuesta
...
for x in range(0, numero_lineas_nomina):
	...
	#se almacena cargo del empleado
	nomina[x][2] = linea_validada[3]
guardar_log("Nomina cargada en memoria!");
# Fin de ciclo 1

#Ciclo 2 para hacer la suma total de la nomina y la nomina por cargos
...
cantidad_tecnicos=0
suma_tecnologo=0
cantidad_tecnologos=0
suma_lider=0
cantidad_lideres=0
suma_gerente=0
cantidad_gerentes=0
#Se inicializan los promediadores
promedio_nomina=0
promedio_nomina_gerentes=0
promedio_nomina_ingenieros=0
promedio_nomina_lideres=0
promedio_nomina_tecnicos=0
promedio_nomina_tecnologos=0
for l in range(0,numero_lineas_nomina):
	suma_nomina = suma_nomina + nomina[l][1]
	if (nomina[l][2].startswith("ingeniero")):
		suma_ingeniero = suma_ingeniero + nomina[l][1]
		cantidad_ingenieros=cantidad_ingenieros+1
	if (nomina[l][2].startswith("tecnico")):
		suma_tecnico = suma_tecnico + nomina[l][1]
		cantidad_tecnicos=cantidad_tecnicos+1
	if (nomina[l][2].startswith("tecnologo")):
		suma_tecnologo = suma_tecnologo + nomina[l][1]
		cantidad_tecnologos=cantidad_tecnologos+1
	if (nomina[l][2].startswith("lider")):
		suma_lider = suma_lider + nomina[l][1]
		cantidad_lideres=cantidad_lideres+1
	if (nomina[l][2].startswith("gerente")):
		suma_gerente = suma_gerente + nomina[l][1]
		cantidad_gerentes=cantidad_gerentes+1
#Fin ciclo 2

#Calculando los promedios
salario_promedio=suma_nomina/numero_lineas_nomina
try:
	promedio_nomina_ingenieros=suma_ingeniero/cantidad_ingenieros

except ZeroDivisionError:
	guardar_log("No hay ingenieros en la empresa")
try:
	promedio_nomina_tecnicos=suma_tecnico/cantidad_tecnicos
except ZeroDivisionError:
	guardar_log("No hay tecnicos en la empresa")
try:
	promedio_nomina_tecnologos=suma_tecnologo/cantidad_tecnologos
except ZeroDivisionError:
	guardar_log("no hay tecnologos en la empresa")
try:
	promedio_nomina_lideres=suma_lider/cantidad_lideres
except ZeroDivisionError:
	guardar_log("no hay lideres en la empresa")
try:
	promedio_nomina_gerentes=suma_gerente/cantidad_gerentes
except ZeroDivisionError:
	guardar_log("no hay gerentes en la empresa")

#convertir las variables de suma en strings para poderlas escribir en el archivo de liquidacion
suma_nomina=str(suma_nomina)
suma_ingeniero=str(suma_ingeniero)
suma_tecnico=str(suma_tecnico)
suma_tecnologo=str(suma_tecnologo)
suma_lider=str(suma_lider)
suma_gerente=str(suma_gerente)
#convertir las variables de primedios en strings para poderlas escribir en el archivo de liquidacion
salario_promedio=str(salario_promedio)
promedio_nomina_gerentes=str(promedio_nomina_gerentes)
promedio_nomina_ingenieros=str(promedio_nomina_ingenieros)
promedio_nomina_lideres=str(promedio_nomina_lideres)
promedio_nomina_tecnicos=str(promedio_nomina_tecnicos)
promedio_nomina_tecnologos=str(promedio_nomina_tecnologos)
...
#se escribe la primera linea del archivo de liquidacion con las sumas.
escribir_linea_archivo(nombre_archivo_liquidacion, "La suma del pago de todos los empleados es: $"+suma_nomina+" | "+"El pago total para ingenieros es: $"+suma_ingeniero+" | "+"El pago total para tecnicos es: $"+suma_tecnico+" | "+"El pago total para tecnologos es: $"+suma_tecnologo+" | "+"El pago total para lideres es: $"+suma_lider+" | "+"El pago total para gerentes es: $"+suma_gerente+"\n")
#Se escribe la segunda linea del archivo de liquidacion con el salario promedio de la empresa
escribir_linea_archivo(nombre_archivo_liquidacion, "El salario promedio para todos los empleados de la empresa es: $"+salario_promedio+"\n")

#Se escribe la tercera linea del archivo de liquidacion con el promedio de sueldo por cargos
escribir_linea_archivo(nombre_archivo_liquidacion, "El salario promedio para ingenieros es: $"+promedio_nomina_ingenieros+" | El salario promedio para tecnicos es: $"+promedio_nomina_tecnicos+" | El salario promedio para tecnologos es: $"+promedio_nomina_tecnologos+" | El salario promedio para lideres es: $"+promedio_nomina_lideres+" | El salario promedio para gerentes es: $"+promedio_nomina_gerentes+"\n")
# Ciclo 4 para guardar liquidacion en archivo liquidacion.txt.
