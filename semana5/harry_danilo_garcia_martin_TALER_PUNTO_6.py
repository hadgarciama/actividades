...
Programa para calcular la nomina de una empresa. Lee los nombres, salarios y cargos desde un archivo de texto cuyo nombre se ingresa como argumento en la linea de comandos.Luego suma la nomina total de la empresa y suma la nomina por cargos.Tambien calcula promedio de suledos y promedio de sueldos por area.El formato de cada linea de los archivos donde se guardan los porcentajes debe ser "key=value"
...
# Validar que los archivo de nomina,apropiaciones y deducciones suministrados como argumento tenga extension .txt.
# Validacion 2 (Va2)
...
#Variable que almacena los porcentajes de apropiaciones
array_apropiaciones=tuple(leer_lineas_archivo(nombre_archivo_apropiaciones))
cantidad_porcentajes_apropiaciones=len(array_apropiaciones)
porcentaje_apropiaciones=[columnas for columnas in range(cantidad_porcentajes_apropiaciones)]
for x1 in range(0,cantidad_porcentajes_apropiaciones):
	split_fila_array_apropiaciones=array_apropiaciones[x1].split("=")
	porcentaje_apropiaciones[x1]=float(split_fila_array_apropiaciones[1])
print porcentaje_apropiaciones

# Modifcar porcentaje segun el tipo de riesgo definido en: https://www.positiva.gov.co/ARL/Paginas/default.aspx
porcentaje_riesgos = porcentaje_apropiaciones[2]

#Variable que almacena los porcentajes de deducciones
array_deducciones=tuple(leer_lineas_archivo(nombre_archivo_deducciones))
cantidad_porcentajes_deducciones=len(array_deducciones)
porcentajes_deducciones=[columnas for columnas in range(cantidad_porcentajes_deducciones)]
for x2 in range(0,cantidad_porcentajes_deducciones):
	split_fila_array_deducciones=array_deducciones[x2].split("=")
	porcentajes_deducciones[x2]=float(split_fila_array_deducciones[1])
print porcentajes_deducciones
...
for z in range(0, numero_lineas_nomina):
	...
	aporte_salud_empresa = porcentaje_apropiaciones[0]*salario_base

	# Porcentaje aporte de pension realizado por la empresa 12%
	guardar_log("Calculando aporte pension empresa ...");
	aporte_pension_empresa = porcentaje_apropiaciones[1]*salario_base
	...
	aporte_parafiscales_sena = porcentaje_apropiaciones[5]*salario_base

	# Porcentaje aporte parafiscal para ICBF realizado por la empresa 3%
	guardar_log("Calculando aporte parafiscales ICBF ...");
	aporte_parafiscales_icbf = porcentaje_apropiaciones[4]*salario_base
	...
	aporte_parafiscales_cajas = porcentaje_apropiaciones[3]*salario_base
	...
	aporte_salud_empleado = porcentajes_deducciones[0]*salario_base
	...
	aporte_pension_empleado = porcentajes_deducciones[1]*salario_base
